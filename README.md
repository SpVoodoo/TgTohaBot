...
Телеграм-бот в стиле "Зек-водолаз", который считает, сколько раз за день написали команду для увеличения счетчика:
...
- `/start` - справка
- `/help` - справка
- `/obideli [число]` - увеличить счетчик (по умолчанию +1)
...
- `/baldness` - случайная картинка + степень (1..5) и процент облысения
- `/gazy [сек]` - запуск раунда "газы" (по умолчанию 30 сек, диапазон 10-300)
- `/mask` - отметить, что выжил в раунде "газы"

## Картинки Для `/baldness`

- Положи картинки в папку `baldness_images/`.
- В `main.py` в словаре `BALDNESS_BY_IMAGE` укажи соответствие:
  `имя_файла -> степень (1..5) и процент облысения`.
- Команда случайно выберет одну из картинок, которые реально есть в папке, и отправит ее с подписью.
